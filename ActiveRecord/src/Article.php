...
class Article
{
    ...
    {
        $this->connection = new PDO('mysql:dbname=magazine;host=localhost','manga','isinteresting');
    ...
    {
        $sql = "SELECT * FROM articles WHERE $columnName = '$value'";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $this->connection->query($sql);
    }
    ...
    {
        $sql = "UPDATE articles SET article_name='$this->name',
